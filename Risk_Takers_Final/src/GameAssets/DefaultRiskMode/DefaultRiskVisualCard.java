...
public class DefaultRiskVisualCard extends VisualCard {
	...
	protected BufferedImage getSymbolicData() {
		if(cardType == CARD_TYPES.EASY_UNIT) 
			return ((DefaultRiskMode)GameController.activeMode).CARD_SYMBOLS[0];
		else if(cardType == CARD_TYPES.MODERATE_UNIT) 
			return ((DefaultRiskMode)GameController.activeMode).CARD_SYMBOLS[1];
		else if(cardType == CARD_TYPES.HARD_UNIT) 
			return ((DefaultRiskMode)GameController.activeMode).CARD_SYMBOLS[2];
		else if(cardType == CARD_TYPES.EXTREME_UNIT) 
			return ((DefaultRiskMode)GameController.activeMode).CARD_SYMBOLS[3];
		return null;
	}
